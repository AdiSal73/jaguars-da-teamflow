...
import { ArrowLeft, Shield, TrendingUp, Target, Zap } from 'lucide-react';
import { Badge } from '@/components/ui/badge';

export default function PlayerKnowledgeDetail() {
  const navigate = useNavigate();
  const urlParams = new URLSearchParams(window.location.search);
  const positionKey = urlParams.get('position') || 'CB';
  ...
  const position = positions[positionKey] || positions.CB;

  const ResponsibilitySection = ({ title, data, icon: Icon, bgColor }) => (
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        <Card className={`border-none shadow-lg ${bgColor}`}>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">Balanced (Organized)</CardTitle>
          </CardHeader>
        ...
        <Card className={`border-none shadow-lg ${bgColor}`}>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">Final Third</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className={`inline-flex items-center gap-3 bg-gradient-to-r ${position.color} text-white px-6 py-3 rounded-2xl shadow-xl mb-4`}>
            <span className="text-4xl">{position.icon}</span>
            <div>
              <h1 className="text-3xl font-bold">{position.title}</h1>
              <p className="text-white/90 text-sm">Complete Position Guide</p>
            </div>
          </div>
        </div>
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {position.traits.map((trait, idx) => (
                  <div key={idx} className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
                    ...
                    <span className="text-sm text-slate-900">{trait}</span>
                  </div>
                ))}
              </div>
            </CardContent>
