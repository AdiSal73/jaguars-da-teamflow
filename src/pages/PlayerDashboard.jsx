...
import { ArrowLeft, User, Mail, Phone, Calendar, Save, ChevronLeft, ChevronRight, TrendingUp, TrendingDown } from 'lucide-react';
...
export default function PlayerDashboard() {
  ...
  React.useEffect(() => {
    if (latestAssessment) {
      setAssessmentForm({
        sprint: latestAssessment.sprint || '',
        vertical: latestAssessment.vertical || '',
        yirt: latestAssessment.yirt || '',
        shuttle: latestAssessment.shuttle || ''
      });
    }
  }, [latestAssessment]);
  ...
  React.useEffect(() => {
    if (latestEvaluation) {
      setEvaluationForm({
        growth_mindset: latestEvaluation.growth_mindset || 5,
        resilience: latestEvaluation.resilience || 5,
        efficiency_in_execution: latestEvaluation.efficiency_in_execution || 5,
        athleticism: latestEvaluation.athleticism || 5,
        team_focus: latestEvaluation.team_focus || 5,
        defending_organized: latestEvaluation.defending_organized || 5,
        defending_final_third: latestEvaluation.defending_final_third || 5,
        defending_transition: latestEvaluation.defending_transition || 5,
        attacking_organized: latestEvaluation.attacking_organized || 5,
        attacking_final_third: latestEvaluation.attacking_final_third || 5,
        attacking_in_transition: latestEvaluation.attacking_in_transition || 5,
        player_strengths: latestEvaluation.player_strengths || '',
        areas_of_growth: latestEvaluation.areas_of_growth || '',
        training_focus: latestEvaluation.training_focus || ''
      });
    }
  }, [latestEvaluation]);
  ...
  const updateAssessmentMutation = useMutation({
    mutationFn: (data) => {
      if (latestAssessment?.id) {
        return base44.entities.PhysicalAssessment.update(latestAssessment.id, data);
      }
    },
    onSuccess: () => queryClient.invalidateQueries(['assessments', playerId])
  });

  const updateEvaluationMutation = useMutation({
    mutationFn: (data) => {
      if (latestEvaluation?.id) {
        return base44.entities.Evaluation.update(latestEvaluation.id, data);
      }
    },
    onSuccess: () => queryClient.invalidateQueries(['evaluations', playerId])
  });

  const handleSaveAll = async () => {
    await updatePlayerMutation.mutateAsync(playerForm);
    await updateTryoutMutation.mutateAsync(tryoutForm);
    if (latestAssessment) await updateAssessmentMutation.mutateAsync(assessmentForm);
    if (latestEvaluation) await updateEvaluationMutation.mutateAsync(evaluationForm);
    setIsEditing(false);
  };
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 bg-red-50 rounded-lg text-center">
                    ...
                    {isEditing ? (
                      <Input type="number" step="0.01" value={assessmentForm.sprint} onChange={e => setAssessmentForm({...assessmentForm, sprint: parseFloat(e.target.value)})} className="h-7 text-sm font-bold text-center" />
                    ) : (
                  ...
                  <div className="p-2 bg-blue-50 rounded-lg text-center">
                    ...
                    {isEditing ? (
                      <Input type="number" value={assessmentForm.vertical} onChange={e => setAssessmentForm({...assessmentForm, vertical: parseInt(e.target.value)})} className="h-7 text-sm font-bold text-center" />
                    ) : (
                  ...
                  <div className="p-2 bg-emerald-50 rounded-lg text-center">
                    ...
                    {isEditing ? (
                      <Input type="number" value={assessmentForm.yirt} onChange={e => setAssessmentForm({...assessmentForm, yirt: parseInt(e.target.value)})} className="h-7 text-sm font-bold text-center" />
                    ) : (
                  ...
                  <div className="p-2 bg-pink-50 rounded-lg text-center">
                    ...
                    {isEditing ? (
                      <Input type="number" step="0.01" value={assessmentForm.shuttle} onChange={e => setAssessmentForm({...assessmentForm, shuttle: parseFloat(e.target.value)})} className="h-7 text-sm font-bold text-center" />
                    ) : (
      ...
      {latestEvaluation && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          <Card className="border-none shadow-lg bg-green-50">
            ...
            <CardContent>
              {isEditing ? (
                <Textarea value={evaluationForm.player_strengths} onChange={e => setEvaluationForm({...evaluationForm, player_strengths: e.target.value})} rows={3} />
              ) : (
          ...
          <Card className="border-none shadow-lg bg-orange-50">
            ...
            <CardContent>
              {isEditing ? (
                <Textarea value={evaluationForm.areas_of_growth} onChange={e => setEvaluationForm({...evaluationForm, areas_of_growth: e.target.value})} rows={3} />
              ) : (
          ...
          <Card className="border-none shadow-lg bg-blue-50">
            ...
            <CardContent>
              {isEditing ? (
                <Textarea value={evaluationForm.training_focus} onChange={e => setEvaluationForm({...evaluationForm, training_focus: e.target.value})} rows={3} />
              ) : (
